...
readingTime: "15"
category: "react"
summary: "In this post, I will explore the reasons behind my decision to reduce the usage of Redux in the majority of my projects and detail the alternatives I opted for instead."
---
...
This issue is fixed in redux when we use useSelector to subscribe to a certain slice of a state. but it's still [an ongoing process for react](https://github.com/reactjs/rfcs/pull/119). Until react releases **_`useSelectedContext`_** hook, we can use [use-context-selector](https://www.npmjs.com/package/use-context-selector) library to subscribe to a certain slice of context. and prevent unnecessary re-renders and performance leaks.
